...
;(function (global, factory) {
  ...
  function within(element, selector) {
    return !isString(selector)
      ? element === selector ||
          (isDocument(selector) ? selector.documentElement : toNode(selector)).contains(
            toNode(element)
    ...
    toNodes(element).forEach(function (ref) {
      ...
      for (var i = 0; i < args.length; i++) {
        supports.Force
          ...
          : classList[
              (!isUndefined(force) ? force : !classList.contains(args[i])) ? 'add' : 'remove'
            ](args[i])
      }
    })
  }
    ...
    if (flip) {
      ...
      each(dirs, function (ref, prop) {
        ...
        boundaries.some(function (boundary) {
          var elemOffset =
            elAttach[dir] === align ? -dim[prop] : elAttach[dir] === alignFlip ? dim[prop] : 0
    ...
    return function (element, value) {
      ...
      if (isUndefined(value)) {
        ...
        css(
          element,
          prop,
          !value && value !== 0 ? '' : +value + boxModelAdjust(prop, element) + 'px'
        )
      }
    }
  }
  ...
  function isInView(element, topOffset, leftOffset) {
    ...
    return (
      intersectRect(client, bounding) || pointInRect({ x: client.left, y: client.top }, bounding)
    )
  }
  ...
  function scrolledOver(element, heightOffset) {
    ...
    return clamp(
      (vh + win.pageYOffset - top) / ((vh + (elHeight - (diff < vp ? diff : 0))) / 100) / 100
    )
  }
    ...
    function registerEvent(component, event, key) {
      ...
      component._events.push(
        on(
          el,
          name,
          !delegate ? null : isString(delegate) ? delegate : delegate.call(component),
          handler,
      ...
      toggleElement: function (targets, show, animate) {
        ...
        return new Promise(function (resolve) {
          ...
          if (
            ...
            var el = toggled[0]
            var inProgress =
              (Animation.inProgress(el) && hasClass(el, 'uk-animation-leave')) ||
  ...
  var FormCustom = {
    ...
    computed: {
      ...
      target: function (ref, $el) {
        ...
        return (
          target &&
          ((target === true && this.input.parentNode === $el && this.input.nextElementSibling) ||
            query(target, $el))
        )
      }
    },
    ...
    update: [
      {
        read: function (ref) {
          ...
          if (this.masonry && this.length) {
              ...
              translates[i] = row.map(function (_, j) {
                return i === 0
                  ...
                  : toFloat(translates[i - 1][j]) +
                      (height - toFloat(rows[i - 1][j] && rows[i - 1][j].offsetHeight))
              })
            ...
            elHeight =
              maxColumnHeight(rows) + getMarginTop(this.$el, this.margin) * (rows.length - 1)
          }
            ...
            write: function () {
              ...
              this.elements.forEach(function (el) {
                ...
                if (
                  height &&
                  (this$1.forceHeight ||
                    Math.round(height + boxModelAdjust('height', el, 'content-box')) >=
                      el.offsetHeight)
                ) {
  ...
  function match(elements) {
    ...
    heights = elements.map(function (el, i) {
      return heights[i] === max && toFloat(el.style.minHeight).toFixed(2) !== max.toFixed(2)
        ? ''
        : max
    })
    ...
    update: {
      read: function (ref) {
        ...
        if (this.expand) {
          ...
          minHeight =
            height(window) -
              (offsetHeight(document.documentElement) - offsetHeight(this.$el)) -
              box || ''
        } else {
  ...
  var Navbar = {
    ...
    beforeConnect: function () {
      ...
      this.dropbar =
        dropbar &&
        (query(dropbar, this.$el) || $('+ .uk-navbar-dropbar', this.$el) || $('<div></div>'))
    ...
    events: [
...

      {
        ...
        handler: function (_, drop) {
          ...
          if (dir === 'bottom') {
            this.transitionTo(
              $el.offsetHeight + toFloat(css($el, 'marginTop')) + toFloat(css($el, 'marginBottom')),
              $el
            )
          }
        }
    ...
    methods: {
      getActive: function () {
        ...
        var active = ref[0]
        return (
          active && includes(active.mode, 'hover') && within(active.toggle.$el, this.$el) && active
        )
      },
    ...
    update: {
      read: function () {
        ...
        return {
          ...
          max: Math.max(
            150,
            height(this.container) - (offset(this.content).height - height(this.$el))
          )
        }
      },
  ...
  var Scroll = {
    ...
    methods: {
      scrollTo: function (el) {
        ...
        var step = function () {
          var currentY =
            startY + (target - startY) * ease(clamp((Date.now() - start) / this$1.duration))
...

      {
        read: function (data) {
          ...
          this.targets.every(function (el, i) {
            ...
            if (
              !this$1.overflow &&
              ((i === 0 && top > scroll) || (last && top + el.offsetTop < scroll))
            ) {
              return false
            }
        ...
        write: function (ref) {
          ...
          if (active) {
            trigger(this.$el, 'active', [
              active,
              addClass(this.closest ? closest(active, this.closest) : active, this.cls)
            ])
          }
        },
  ...
  var Sticky = {
    ...
    connected: function () {
      this.placeholder =
        $('+ .uk-sticky-placeholder', this.$el) || $('<div class="uk-sticky-placeholder"></div>')
      this.isFixed = false
      this.isActive = false
    },
  ...
  var Switcher = {
    ...
    methods: {
      index: function () {
        return (
          !isEmpty(this.connects) && index(filter(this.connects[0].children, '.' + this.cls)[0])
        )
      },
    ...
    methods: {
      animate: function (action) {
        ...
        propsFrom = propsTo.map(function (props, i) {
          var from =
            children[i].parentNode === this$1.target ? propsFrom[i] || getProps(children[i]) : false
        ...
        return Promise.all(
          children
            ...
            .concat(Transition.start(this.target, { height: newHeight }, this.animation, 'ease'))
        ).then(function () {
          children.forEach(function (el, i) {
      ...
      move: function (e) {
        ...
        if (this.dragging) {
          ...
          if (this.index === this.prevIndex) {
            ...
            var dirChange =
              (isRtl ? this.dir * (isRtl ? 1 : -1) : this.dir) < 0 === this.prevPos > this.pos
      ...
      updateNav: function () {
        ...
        this.navItems.forEach(function (el) {
          ...
          toggleClass(
            ...
            'uk-invisible',
            this$1.finite &&
              ((cmd === 'previous' && i === 0) || (cmd === 'next' && i >= this$1.maxIndex))
          )
        })
      }
    }
...

      {
        ...
        handler: function (_, item) {
          ...
          if (type === 'image' || source.match(/\.(jp(e)?g|png|gif|svg|webp)($|\?)/i)) {
            ...
            (matches =
              source.match(/\/\/.*?youtube(-nocookie)?\.[a-z]+\/watch\?v=([^&\s]+)/) ||
            ...
            var setIframe = function (width, height) {
              ...
              return this$1.setItem(
                item,
                getIframe(
                  'https://www.youtube' + (matches[1] || '') + '.com/embed/' + id,
                  width,
                  height,
            ...
            getImage('https://img.youtube.com/vi/' + id + '/maxresdefault.jpg').then(function (
              ref
            ) {
              var width = ref.width
              var height = ref.height
  ...
  var Lightbox = {
    ...
    methods: {
      show: function (index) {
        ...
        this.panel =
          this.panel ||
          this.$create('lightboxPanel', assign({}, this.$props, { items: this.items }))
  ...
  function getValue(steps, percent, digits) {
    ...
    var p = ref[2]
    return (
      isNumber(start) ? start + Math.abs(start - end) * p * (start < end ? 1 : -1) : +end
    ).toFixed(digits)
  }
    ...
    return {
      ...
      translate: function (percent) {
        ...
        css(
          ...
          'transform',
          translate(
            clamp(-to + (distance - distance * percent), -getWidth(list), bounds(list).width) *
              (isRtl ? -1 : 1),
            'px'
          )
        )
  ...
  function getElLeft(el, list) {
    return (
      (position(el).left + (isRtl ? bounds(el).width - bounds(list).width : 0)) * (isRtl ? -1 : 1)
    )
  }
  ...
  var Slider$1 = {
    ...
    computed: {
      ...
      finite: function (ref) {
        ...
        return (
          finite ||
          getWidth(this.list) < bounds(this.list).width + getMaxWidth(this.list) + this.center
        )
      },
        ...
        $$('[' + this.attrItem + '],[data-' + this.attrItem + ']', this.$el).forEach(function (el) {
          ...
          this$1.maxIndex &&
            toggleClass(
              ...
              'uk-hidden',
              isNumeric(index) &&
                ((this$1.sets && !includes(this$1.sets, toFloat(index))) || index > this$1.maxIndex)
            )
        })
      ...
      reorder: function () {
        ...
        this.slides.forEach(function (slide, i) {
          return css(
            slide,
            'order',
            this$1.dir > 0 && i < index ? 1 : this$1.dir < 0 && i >= this$1.index ? -1 : ''
          )
        })
  ...
  var Sortable = {
    ...
    methods: {
      ...
      start: function (e) {
        this.drag = append(
          this.$container,
          this.placeholder.outerHTML.replace(/^<li/i, '<div').replace(/li>$/i, 'div>')
        )
      ...
      getSortable: function (element) {
        return (
          element &&
          (this.$getComponent(element, 'sortable') || this.getSortable(element.parentNode))
        )
      }
    }
  }
  ...
  function trackScroll(ref) {
    ...
    scrollParents(document.elementFromPoint(x - window.pageXOffset, y - window.pageYOffset)).some(
      function (scrollEl) {
